...
BOOL CGvisR2R_PunchDoc::LoadMySpec() // 상면 기준으로 마킹부 상면작업 로컬데이타만.
{
	...
	if (!WorkingInfo.CurrModel.sModel.IsEmpty() && !WorkingInfo.CurrModel.sLot.IsEmpty())
	{
		if (!m_pSpecLocal)
		{
			m_pSpecLocal = new CMySpec(WorkingInfo.CurrModel.sModel, WorkingInfo.CurrModel.sLot);
			return FALSE;
		}
		else
		{
...
BOOL CGvisR2R_PunchDoc::LoadWorkingInfo()
{
	...
	{
		pView->ClrDispMsg(); AfxMessageBox(_T("Model이 설정되어 있지 않습니다."), MB_ICONWARNING | MB_OK);
...
void CGvisR2R_PunchDoc::SaveWorkingInfo()
{
	...
	//sData = WorkingInfo.CurrModel.sLot;
...
CString CGvisR2R_PunchDoc::GetCamPxlRes()
{
	...
	if (pDoc->WorkingInfo.System.sPathCamSpecDir.Right(1) != "\\")
		sPath.Format(_T("%s\\%s\\%s.mst"), pDoc->WorkingInfo.System.sPathCamSpecDir, pDoc->WorkingInfo.CurrModel.sModel, pDoc->WorkingInfo.CurrModel.sLot);
	else
		sPath.Format(_T("%s%s\\%s.mst"), pDoc->WorkingInfo.System.sPathCamSpecDir, pDoc->WorkingInfo.CurrModel.sModel, pDoc->WorkingInfo.CurrModel.sLot);
#endif
...
BOOL CGvisR2R_PunchDoc::GetAoiInfoUp(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (strModel.IsEmpty() || strLayer.IsEmpty() || strLot.IsEmpty())
	{
	...
	if (WorkingInfo.CurrModel.sModel != strModel)
	{
		if (WorkingInfo.CurrModel.sModel.IsEmpty()) // 초기화 이후 처음 Model 갱신
			bUpdate = TRUE;
		else
			pView->m_bChangeModel = TRUE;

		WorkingInfo.NextModel.sModel = strModel;
	}


	if (WorkingInfo.CurrModel.sLot != strLot)
	{
		if (WorkingInfo.CurrModel.sLot.IsEmpty()) // 초기화 이후 처음 Lot 갱신
			bUpdate = TRUE;
		else
			pView->m_bChangeLot = TRUE;

		WorkingInfo.NextModel.sLot = strLot;
	}

	if (WorkingInfo.CurrModel.sLayerUp != strLayer)
	{
		if (WorkingInfo.CurrModel.sLayerUp.IsEmpty()) // 초기화 이후 처음 Layer 갱신
			bUpdate = TRUE;
		else
			pView->m_bChangeLayer = TRUE;

		WorkingInfo.NextModel.sLayerUp = strLayer;
	}

	//if (WorkingInfo.CurrModel.sModel != Status.PcrShare[0].sModel || WorkingInfo.CurrModel.sLot != Status.PcrShare[0].sLayer || pView->m_bInitAutoLoadMstInfo)
	//{
	//	bUpdate = TRUE;
	//	WorkingInfo.CurrModel.sModel = Status.PcrShare[0].sModel;
	//	WorkingInfo.CurrModel.sLot = Status.PcrShare[0].sLayer;
	//
	//	if (m_bBufEmptyF[0])
	//	{
	//		if (!m_bBufEmpty[0])
	...
	// Change Model now.
	if (bUpdate) // 저장 경로를 설정
	{
		WorkingInfo.NextModel.sModel = strModel;
		WorkingInfo.NextModel.sItsCode = sItsCode;
		WorkingInfo.NextModel.sLot = strLot;
		WorkingInfo.NextModel.sLayerUp = strLayer;
		//WorkingInfo.NextModel.sLayerDn = strLayer;
...
BOOL CGvisR2R_PunchDoc::GetAoiInfoDn(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (WorkingInfo.CurrModel.sLayerDn != strLayer)
	{
		WorkingInfo.NextModel.sLayerDn = strLayer;
	}
	...
	if (WorkingInfo.CurrModel.sLayerDn != strLayer)
	{
		if (WorkingInfo.CurrModel.sLayerDn.IsEmpty()) // 초기화 이후 처음 Layer 갱신
			bUpdate = TRUE;
		else
			pView->m_bChangeLayer = TRUE;
	...
	//if (WorkingInfo.LastJob.sModelDn != Status.PcrShare[1].sModel || WorkingInfo.CurrModel.sLayerDn != Status.PcrShare[1].sLayer)
	//{
	//	bUpdate = TRUE;
	//	WorkingInfo.LastJob.sModelDn = Status.PcrShare[1].sModel;
	//	WorkingInfo.CurrModel.sLayerDn = Status.PcrShare[1].sLayer;
	//
	//	if (m_bBufEmptyF[1])
	//	{
	//		if (!m_bBufEmpty[1])
...
int CGvisR2R_PunchDoc::LoadPCRUp(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	if (WorkingInfo.CurrModel.sModel != strModel)
	{
		if (WorkingInfo.CurrModel.sModel.IsEmpty()) // 초기화 이후 처음 Model 갱신
		{
			bUpdate = TRUE;
			if (pView->m_pEngrave)
				pView->m_pEngrave->SetModelName(strModel);
		}
		else
			pView->m_bChangeModel = TRUE;
	...
	//if (WorkingInfo.CurrModel.sItsCode != sItsCode)
	//{
	//	if (WorkingInfo.CurrModel.sItsCode.IsEmpty()) // 초기화 이후 처음 ItsCode 갱신
	//		bUpdate = TRUE;
	//	WorkingInfo.NextModel.sItsCode = sItsCode;
	//}
	...
	if (WorkingInfo.CurrModel.sLot != strLot)
	{
		...
		WorkingInfo.NextModel.sLot = strLot;
		if (pView->m_pEngrave)
			pView->m_pEngrave->SetLotName(strLot);
	}
	...
	if (WorkingInfo.CurrModel.sLayerUp != strLayer)
	{
		...
		WorkingInfo.NextModel.sLayerUp = strLayer;
		if (pView->m_pEngrave)
			pView->m_pEngrave->SetLayerUpName(strLayer);
	}
	...
	//if (WorkingInfo.CurrModel.sLot != strLot || WorkingInfo.LastJob.sEngItsCode != sItsCode)
	//{
	//	if (WorkingInfo.CurrModel.sLot != strLot)
	//	{
	//		WorkingInfo.CurrModel.sLot = strLot;
	//		if(pView->m_pEngrave)
	...
	//}
	//
	//if (WorkingInfo.CurrModel.sModel != strModel || WorkingInfo.CurrModel.sLot != strLayer)
	//{
	//	if (WorkingInfo.CurrModel.sModel != strModel)
	//	{
	//		WorkingInfo.CurrModel.sModel = strModel;
	//		if (pView->m_pEngrave)
	//			pView->m_pEngrave->SetModelName();
	//	}
	//	if (WorkingInfo.CurrModel.sLot != strLayer)
	//	{
	//		WorkingInfo.CurrModel.sLot = strLayer;
	//		if (pView->m_pEngrave)
	...
	if (nTotDef > 0)
	{
		for (i = 0; i < nTotDef; i++)
		{
			...
			pView->m_pMgrReelmap->m_pPcsRgn->GetMkMatrix(m_pPcr[0][nIdx]->m_pDefPcs[i], nC, nR);
...
int CGvisR2R_PunchDoc::LoadPCRDn(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	if (WorkingInfo.CurrModel.sModel != strModel)
	{
		if (WorkingInfo.CurrModel.sModel.IsEmpty())
		{
			bUpdate = TRUE;
			if (pView->m_pEngrave)
				pView->m_pEngrave->SetModelName(strModel);
	...
	if (WorkingInfo.CurrModel.sLayerDn != strLayer)
	{
		...
		WorkingInfo.NextModel.sLayerDn = strLayer;
		if (pView->m_pEngrave)
			pView->m_pEngrave->SetLayerDnName(strLayer);
	}
	...
	//		WorkingInfo.CurrModel.sLot = strLot;
	//		if (pView->m_pEngrave)
	//			pView->m_pEngrave->SetLotDnName();
	//	}
	//	bUpdate = TRUE;
	//	m_sItsCode = WorkingInfo.LastJob.sEngItsCode = sItsCode;
	//}
	//
	//if (WorkingInfo.LastJob.sModelDn != strModel || WorkingInfo.CurrModel.sLayerDn != strLayer)
	//{
	//	if (WorkingInfo.LastJob.sModelDn != strModel)
	//	{
	...
	//	if (WorkingInfo.CurrModel.sLayerDn != strLayer)
	//	{
	//		WorkingInfo.CurrModel.sLayerDn = strLayer;
	//		if (pView->m_pEngrave)
	...
	if (nTotDef > 0)
	{
		for (i = 0; i < nTotDef; i++)
		{
			...
			pView->m_pMgrReelmap->m_pPcsRgn->GetMkMatrix(Rotate180(m_pPcr[1][nIdx]->m_pDefPcs[i]), nC, nR);
...
BOOL CGvisR2R_PunchDoc::CopyDefImgUp(int nSerial, CString sNewLot)
{
	...
	if (sNewLot.IsEmpty())
		sLot = WorkingInfo.CurrModel.sLot;
	else
		sLot = sNewLot;
...
void CGvisR2R_PunchDoc::MakeImageDirDn(int nSerial)
{
	...
	if (WorkingInfo.System.sPathOldFile.Right(1) != "\\")
		strMakeFolderPath.Format(_T("%s\\%s\\%s\\%s"), WorkingInfo.System.sPathOldFile,
			WorkingInfo.CurrModel.sModel,
			sLot,
			WorkingInfo.CurrModel.sLayerDn);
	else
		strMakeFolderPath.Format(_T("%s%s\\%s\\%s"), WorkingInfo.System.sPathOldFile,
			WorkingInfo.CurrModel.sModel,
			sLot,
...
{
	CString sData, sPath = PATH_WORKING_INFO;
	sData = WorkingInfo.CurrModel.sModel;
	::WritePrivateProfileString(_T("Last Job"), _T("Model Name"), sData, sPath);
...
void CGvisR2R_PunchDoc::SetModelInfoDn()
{
	...
	sData = WorkingInfo.CurrModel.sLayerDn;
	::WritePrivateProfileString(_T("Last Job"), _T("LayerDn Name"), sData, sPath);
...
void CGvisR2R_PunchDoc::WriteElecData(CString sData)
{
	...
	sLot = WorkingInfo.CurrModel.sLot;
	if (WorkingInfo.System.sPathOldFile.Right(1) != "\\")
		strDestPath.Format(_T("%s\\%s\\%s\\%s.txt"), WorkingInfo.System.sPathOldFile, WorkingInfo.CurrModel.sModel,
			sLot, sLot);
	else
		strDestPath.Format(_T("%s%s\\%s\\%s.txt"), WorkingInfo.System.sPathOldFile, WorkingInfo.CurrModel.sModel,
			sLot, sLot);
...
{
	return WorkingInfo.CurrModel.nTestMode;
}

int CGvisR2R_PunchDoc::GetNextTestMode()
{
	return WorkingInfo.NextModel.nTestMode;
}
...
void CGvisR2R_PunchDoc::SetTestMode(int nMode)
{
	...
	{
		WorkingInfo.CurrModel.nTestMode = nMode; // MODE_NONE = 0, MODE_INNER = 1, MODE_OUTER = 2 .
		WorkingInfo.NextModel.nTestMode = nMode; // MODE_NONE = 0, MODE_INNER = 1, MODE_OUTER = 2 .
...
BOOL CGvisR2R_PunchDoc::GetCurrentInfoEng()
{
	...
	if (bDualTest)
	{
		WorkingInfo.CurrModel.sLot = WorkingInfo.CurrModel.sLot;
...
BOOL CGvisR2R_PunchDoc::GetItsSerialInfo(int nItsSerial, BOOL &bDualTest, CString &sLot, CString &sLayerUp, CString &sLayerDn, int nOption)		// 내층에서의 ITS 시리얼의 정보
{
	...
	Path[1] = WorkingInfo.CurrModel.sModel;
	Path[2] = m_sItsCode;
	...
	if (nOption == 0 || nOption == 2)
	{
		...
		WorkingInfo.CurrModel.sLotInner = sLot;
	}
	...
	if (nOption == 0 || nOption == 3)
	{
		...
		WorkingInfo.CurrModel.sLayerUpInner = sLayerUp;
	}
	...
	if (nOption == 0 || nOption == 4)
	{
		...
		WorkingInfo.CurrModel.sLayerDnInner = sLayerDn;
	}

	return TRUE;
}

BOOL CGvisR2R_PunchDoc::GetItsSerialInfoNext(int nItsSerial, BOOL &bDualTest, CString &sLot, CString &sLayerUp, CString &sLayerDn, int nOption)		// 내층에서의 ITS 시리얼의 정보
{
	TCHAR szData[512];
	CString str, sName, sPath, Path[3];

	Path[0] = WorkingInfo.System.sPathItsFile;
	Path[1] = WorkingInfo.NextModel.sModel;
	//Path[1] = m_sEngModel;
	Path[2] = m_sItsCode;
...
void CGvisR2R_PunchDoc::LoadPCRIts00(int nSerial) // 10 -> 외층 : 단면, 내층 : 단면
{
	pView->ClrDispMsg();
...
void CGvisR2R_PunchDoc::WriteChangedModel()
{
	...
	sData = WorkingInfo.CurrModel.sLayerUp;
	::WritePrivateProfileString(_T("Last Job"), _T("LayerUp Name"), sData, sPath);
...
int CGvisR2R_PunchDoc::GetAoiUpCamMstInfo()
{
	...
	sPath.Format(_T("%s%s\\%s\\%s\\DataOut.ini"), pDoc->WorkingInfo.System.sPathAoiUpVrsData,
		pDoc->WorkingInfo.NextModel.sModel, pDoc->WorkingInfo.NextModel.sLot, pDoc->WorkingInfo.NextModel.sLot);
		
	if (0 < ::GetPrivateProfileString(_T("Region"), _T("Piece Region Type"), NULL, szData, sizeof(szData), sPath))
		pView->m_pMgrReelmapNext->m_pMasterInfo->nOutFileOnAoi = _ttoi(szData);
	else
		pView->m_pMgrReelmapNext->m_pMasterInfo->nOutFileOnAoi = -1;

	return pView->m_pMgrReelmapNext->m_pMasterInfo->nOutFileOnAoi;
}
...
int CGvisR2R_PunchDoc::GetAoiDnCamMstInfo()
{
	...
	sPath.Format(_T("%s%s\\%s\\%s\\DataOut.ini"), pDoc->WorkingInfo.System.sPathAoiDnVrsData,
		pDoc->WorkingInfo.NextModel.sModel, pDoc->WorkingInfo.NextModel.sLayerDn, pDoc->WorkingInfo.NextModel.sLot);
...
BOOL CGvisR2R_PunchDoc::GetAoiInfoUp(int nSerial, stModelInfo& stInfo)
{
	...
	if (WorkingInfo.CurrModel.sModel != strModel)
	{
		if (WorkingInfo.CurrModel.sModel.IsEmpty() && WorkingInfo.NextModel.sModel.IsEmpty()) // 초기화 이후 처음 Model 갱신
			bUpdate = TRUE;
		else
		{
			pView->m_bChangeModel = TRUE;
	...
	if (WorkingInfo.CurrModel.sLot != strLot)
	{
		if (WorkingInfo.CurrModel.sLot.IsEmpty() && WorkingInfo.NextModel.sLot.IsEmpty()) // 초기화 이후 처음 Lot 갱신
			bUpdate = TRUE;
		else
		{
			pView->m_bChangeLot = TRUE;
	...
	if (WorkingInfo.CurrModel.sLayerUp != strLayer)
	{
		if (WorkingInfo.CurrModel.sLayerUp.IsEmpty() && WorkingInfo.NextModel.sLayerUp.IsEmpty()) // 초기화 이후 처음 Layer 갱신
			bUpdate = TRUE;
		else
		{
			pView->m_bChangeLayer = TRUE;
...
BOOL CGvisR2R_PunchDoc::GetAoiInfoDn(int nSerial, stModelInfo& stInfo)
{
	BOOL bDualTest = pDoc->WorkingInfo.LastJob.bDualTest;
	if (!bDualTest)
		return TRUE;
	...
	if (WorkingInfo.CurrModel.sLayerDn != strLayer)
	{
		if (WorkingInfo.CurrModel.sLayerDn.IsEmpty() && WorkingInfo.NextModel.sLayerDn.IsEmpty()) // 초기화 이후 처음 Layer 갱신
			bUpdate = TRUE;
		else
		{
			pView->m_bChangeLayer = TRUE;
...
BOOL CGvisR2R_PunchDoc::CheckModelInfo(stModelInfo& stInfo)
{
	BOOL bUpdate = FALSE;

	if (WorkingInfo.CurrModel.sModel != stInfo.sModel)
	{
		bUpdate = TRUE;
	}

	//if (WorkingInfo.CurrModel.sItsCode != stInfo.sItsCode)
	//{
	//	bUpdate = TRUE;
	//}

	if (WorkingInfo.CurrModel.sLot != stInfo.sLot)
	{
		bUpdate = TRUE;
	}

	if (WorkingInfo.CurrModel.sLayerUp != stInfo.sLayer)
	{
		bUpdate = TRUE;
	}
...
void CGvisR2R_PunchDoc::ChangeWorkingInfoModel()
{
	WorkingInfo.CurrModel.sModel = WorkingInfo.NextModel.sModel;
	WorkingInfo.CurrModel.sItsCode = WorkingInfo.NextModel.sItsCode;
	WorkingInfo.CurrModel.sLot = WorkingInfo.NextModel.sLot;
