	...
	: CDialog(CDlgMenu04::IDD, pParent)
{
	...
	m_bEngBuffHommingF = FALSE;
	m_bEngBuffHommingDoneF = FALSE;
	m_bEngBuffInitMvF = FALSE;
	m_bEngBuffInitMvDoneF = FALSE;
	m_bEngBuffInitPosSaveF = FALSE;
}
...
void CDlgMenu04::DispBufEnc()
{
	...
	if (pView && pView->m_pEngrave)
	{
		...
		{
			m_bEngBuffHommingF = FALSE;							// 각인부 버퍼롤러 홈동작 ON (PLC가 홈동작 완료 후 OFF)
		}
...
//				pView->MpeWrite(_T("MB44017A"), 1);		// 각인부 버퍼 초기위치 이동(PC가 ON, PLC가 OFF)
		...
		if (pDoc->WorkingInfo.Motion.bEngBuffInitPosSave && !m_bEngBuffInitPosSaveF)
		{
			m_bEngBuffInitPosSaveF = TRUE;
			EngBuffInitPosSave();
		}
		else if (!pDoc->WorkingInfo.Motion.bEngBuffInitPosSave && m_bEngBuffInitPosSaveF)
		{
			m_bEngBuffInitPosSaveF = FALSE;
		}
...
void CDlgMenu04::OnStc162() 
{
	...
	GetDlgItem(IDC_STC_162)->GetWindowText(sData);
 	pDoc->SetSeparateDist(_tstof(sData));
	pView->SetLotLastShot();
...
void CDlgMenu04::EngBuffInitPosSave()
{
	// TODO: Add your control notification handler code here
	if (!pView || !pDoc || !pDoc->m_pMpeData)
		return;
