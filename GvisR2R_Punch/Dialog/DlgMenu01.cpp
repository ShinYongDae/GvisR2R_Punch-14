...
void CDlgMenu01::DispMkInfoUp()
{
	...
	if (nMaxMaxDispDefImg > 0 && pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef - nMaxMaxDispDefImg > 0)
		m_nIdxDef[0] = pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef - nMaxMaxDispDefImg; // 불량이미지 인덱스.
	else
	...
	if(pView->m_pMgrReelmap->m_pPcr[0])
	{
		...
		{
			m_nDef[0] = pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef; // m_nDef : m_nIdxMkInfo + Display Def Num.
			pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotRealDef = 0;
		}
	}
#endif
}
...
void CDlgMenu01::DispMkInfoDn()
{
	...
	if(nMaxMaxDispDefImg > 0 && pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotDef - nMaxMaxDispDefImg > 0)
		m_nIdxDef[1] = pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotDef - nMaxMaxDispDefImg; // 불량이미지 인덱스.
	else
	...
	if(pView->m_pMgrReelmap->m_pPcr[1])
	{
		...
		{
			m_nDef[1] = pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotDef; // m_nDef : m_nIdxMkInfo + Display Def Num.
			pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotRealDef = 0;
		}
	}
#endif
}
	...
	if(bDualTest)
	{
		if(pView->m_pVision[0])
		{
			...
			if(pView->m_pMgrReelmap->m_pPcr[0])
			{
				if(pView->m_pMgrReelmap->m_pPcr[0][nIdx])
				{
					if(pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_pImg)
					{
						if(pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_pMk[m_nIdxDef[0]] != -2) // -2 (NoMarking)
						{	
							if(m_nIdxDef[0] < pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef)
							{
								if(m_nIdxMkInfo[0] >= MENU01_STC_DEFINFO_HARF)
									ShiftDefInfoUp();
 								pView->m_pVision[0]->ShowDispCad(nIdxMkInfo, nSerial, 0, m_nIdxDef[0]);
								pView->m_pVision[0]->ShowOvrCad(nIdxMkInfo, nSerial);
								nDefImg = pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_pImg[m_nIdxDef[0]]; // 화면에 표시할 불량이미지 인덱스
								...
								(pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotRealDef)++;
							}
							else
								m_nIdxMkInfo[0]++;
						}
						...
						{	
							if(m_nIdxDef[0] < pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef)
							{
								if(m_nIdxMkInfo[0] >= MENU01_STC_DEFINFO_HARF*2)
 								...
 								pView->m_pVision[0]->ShowDispCad(nIdxMkInfo, nSerial, 0, m_nIdxDef[0]);
								pView->m_pVision[0]->ShowOvrCad(nIdxMkInfo, nSerial);
								nDefImg = pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_pImg[m_nIdxDef[0]];
								pView->m_pVision[0]->ShowDispDef(nIdxMkInfo, nSerial, 0, nDefImg);
...
void CDlgMenu01::DispMkInfoDn(int nSerial)
{
	...
	if(pView->m_pVision[1])
	{
		...
		if(pView->m_pMgrReelmap->m_pPcr[1])
		{
			if(pView->m_pMgrReelmap->m_pPcr[1][nIdx])
			{
				if(pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_pImg)
				{
					if(pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_pMk[m_nIdxDef[1]] != -2) // -2 (NoMarking)
					{	
						if(m_nIdxDef[1] < pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotDef)
						{
 							if(m_nIdxMkInfo[1] >= MENU01_STC_DEFINFO_HARF)
								ShiftDefInfoDn();
 							pView->m_pVision[1]->ShowDispCad(nIdxMkInfo, nSerial, 1, m_nIdxDef[1]);
							pView->m_pVision[1]->ShowOvrCad(nIdxMkInfo, nSerial);
							nDefImg = pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_pImg[m_nIdxDef[1]];
							...
							m_nIdxMkInfo[1]++;
							m_nIdxDef[1]++;
							(pView->m_pMgrReelmap->m_pPcr[1][nIdx]->m_nTotRealDef)++;
						}
						else
							m_nIdxMkInfo[1]++;
					}
...
void CDlgMenu01::ShowDefInfoUp(int nIdx) // nIdx : 0 ~ 11 (12ea)
{
	...
	if(pView->m_pMgrReelmap->m_pPcr[0][nPcrIdx]->m_pMk[m_nIdxDef[0]] == -2) // -2 (NoMarking)
		return;
	int nPcsIdx = pView->m_pMgrReelmap->m_pPcr[0][nPcrIdx]->m_pDefPcs[m_nIdxDef[0]]; //m_pImg[m_nIdxDef];
	nDefCode = pView->m_pMgrReelmap->m_pPcr[0][nPcrIdx]->m_pDefType[m_nIdxDef[0]];
...
void CDlgMenu01::ShowDefInfoDn(int nIdx) // nIdx : 0 ~ 11 (12ea)
{
	...
	if(pView->m_pMgrReelmap->m_pPcr[1][nPcrIdx]->m_pMk[m_nIdxDef[1]] == -2) // -2 (NoMarking)
		return;
	int nPcsIdx = pView->m_pMgrReelmap->m_pPcr[1][nPcrIdx]->m_pDefPcs[m_nIdxDef[1]]; //m_pImg[m_nIdxDef];
	nDefCode = pView->m_pMgrReelmap->m_pPcr[1][nPcrIdx]->m_pDefType[m_nIdxDef[1]];
	rgbDef = pDoc->m_pReelMap->m_rgbDef[nDefCode];
	//if(pDoc->m_Master[0].m_pPcsRgn)
	//	pDoc->m_Master[0].m_pPcsRgn->GetMkMatrix(nPcsIdx, nStrip, nCol, nRow);	
	if (pView->m_pMgrReelmap && pView->m_pMgrReelmap->m_pPcsRgn)
...
BOOL CDlgMenu01::SaveDefImgPosUp(int nSerial, int nIdxMkInfo, int nIdxImg) // (nSerial, 화면의 IDC 인덱스, 불량이미지 인덱스)
{
	...
	int nPcsIdx = pView->m_pMgrReelmap->m_pPcr[0][nPcrIdx]->m_pDefPcs[m_nIdxDef[0]];
	int nDefCode = pView->m_pMgrReelmap->m_pPcr[0][nPcrIdx]->m_pDefType[m_nIdxDef[0]];
	//if (pDoc->m_Master[0].m_pPcsRgn)
	//	pDoc->m_Master[0].m_pPcsRgn->GetMkMatrix(nPcsIdx, nStrip, nCol, nRow);
	if (pView->m_pMgrReelmap && pView->m_pMgrReelmap->m_pPcsRgn)
		pView->m_pMgrReelmap->m_pPcsRgn->GetMkMatrix(nPcsIdx, nStrip, nCol, nRow);
...
BOOL CDlgMenu01::SaveDefImgPosDn(int nSerial, int nIdxMkInfo, int nIdxImg) // (nSerial, 화면의 IDC 인덱스, 불량이미지 인덱스)
{
	...
	int nPcsIdx = pView->m_pMgrReelmap->m_pPcr[1][nPcrIdx]->m_pDefPcs[m_nIdxDef[1]];
	int nDefCode = pView->m_pMgrReelmap->m_pPcr[1][nPcrIdx]->m_pDefType[m_nIdxDef[1]];
	//if (pDoc->m_Master[0].m_pPcsRgn)
	//	pDoc->m_Master[0].m_pPcsRgn->GetMkMatrix(nPcsIdx, nStrip, nCol, nRow);
	if (pView->m_pMgrReelmap && pView->m_pMgrReelmap->m_pPcsRgn)
		pView->m_pMgrReelmap->m_pPcsRgn->GetMkMatrix(nPcsIdx, nStrip, nCol, nRow);
...
void CDlgMenu01::WriteDefInfoDn(int nSerial, int nIdxText, int nIdxDef, int nIdxImg)
{
	...
	sData.Format(_T("%d"), pView->m_pMgrReelmap->m_pPcr[2][nIdx]->m_nTotDef);
...
void CDlgMenu01::OnTimer(UINT_PTR nIDEvent)//(UINT nIDEvent)
{
	...
	if(nIDEvent == TIM_DISP_DEF_IMG) // SetSerial() ---> DispMkInfo() ---> DispMkInfo(m_nSerial);
	{
		...
		{			
			...
			if(m_nIdxDef[0] < m_nDef[0] || m_nIdxDef[1] < m_nDef[1]) // m_nIdxDef(불량이미지 인덱스) , m_nDef[0] = pView->m_pMgrReelmap->m_pPcr[0][nIdx]->m_nTotDef; // m_nDef[up] : 릴맵 화면 표시 인덱스의 Display Def Num.
			{
				if(m_bTIM_DISP_DEF_IMG)
...
void CDlgMenu01::UpdateData()
{
	...
	if(bDualTest)
	{
		myStcData[75].SetText(pDoc->WorkingInfo.CurrModel.sLayerDn);		// 하면레이어
		pDoc->SetMkMenu01(_T("Info"), _T("LayerDn"), pDoc->WorkingInfo.CurrModel.sLayerDn);
	}

	myStcData[1].SetText(pDoc->WorkingInfo.CurrModel.sModel);		// 모델
	pDoc->SetMkMenu01(_T("Info"), _T("Model"), pDoc->WorkingInfo.CurrModel.sModel);
	...
	myStcData[3].SetText(pDoc->WorkingInfo.CurrModel.sLot);			// 로트
	pDoc->SetMkMenu01(_T("Info"), _T("Lot"), pDoc->WorkingInfo.CurrModel.sLot);
	myStcData[2].SetText(pDoc->WorkingInfo.CurrModel.sLayerUp);		// 상면레이어
	pDoc->SetMkMenu01(_T("Info"), _T("LayerUp"), pDoc->WorkingInfo.CurrModel.sLayerUp);
			...
			{ 
				if(IDNO == pView->MsgBox(_T("AOI 상면부터 잔량처리를 하시겠습니까?"), 0, MB_YESNO))
				{
					...
					else // AOI 하면부터 잔량처리
					{
						...
						pView->MpeWrite(pView->Plc.DlgMenu01.LastJob, 1);					// 잔량처리(PC가 On시키고, PLC가 확인하고 Off시킴)-20141031
				...
				else // AOI 상면부터 잔량처리
				{
					...
					try
					{
						...
						pView->MpeWrite(pView->Plc.DlgMenu01.LastJob, 1);				// 잔량처리(PC가 On시키고, PLC가 확인하고 Off시킴)
					}
					catch (_com_error e)
					{
...
void CDlgMenu01::OnChkLotEnd() 
{
	...
	if (bOn && !pView->m_bJoinContinue)
	{
		...
		else
		{
			...
			pView->m_pMpe->Write(_T("MB000000"), 0);			// 이음매처리 (PC가 On시키고, PLC가 확인하고 Off시킴)
...
#ifdef USE_MPE					
				pView->m_pMpe->Write(_T("ML00000"), lOffset);			// 이음매처리 Offset
				pView->m_pMpe->Write(_T("MB000000"), 1);				// 잔량처리(PC가 On시키고, PLC가 확인하고 Off시킴)
#endif
			}
			catch (_com_error e)
			{
...
void CDlgMenu01::ResetLotChange()
{
