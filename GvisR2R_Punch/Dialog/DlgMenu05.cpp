...
{
	m_sModel = pDoc->WorkingInfo.CurrModel.sModel;
	m_sLot = pDoc->WorkingInfo.CurrModel.sLot;
	m_sLayer = pDoc->WorkingInfo.CurrModel.sLayerUp;
}
...
CString CDlgMenu05::DisplayData()
{
	...
	strFileData += pDoc->WorkingInfo.CurrModel.sLayerUp;
	strFileData += _T("\r\n");
	if(bDualTest)
	{
		strFileData += _T("\t하면 레이어 : \t");
		strFileData += pDoc->WorkingInfo.CurrModel.sLayerDn;
		strFileData += _T("\r\n");
	}
	strFileData += _T("\t시작시간 : \t");
